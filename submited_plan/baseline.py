...
def convert_data(data):
    data['time'] = data.context_timestamp.apply(timestamp_datatime())
    data['day'] = data.time.apply(lambda x:int(x[8:10]))
    data['hour'] = data.time.apply(lambda x:int(x[11:13]))
    user_query_day = data.groupby(['user_id','day']).size().reset_index().rename(column={0:'user_query_day'})
    data = pd.merge(data,user_query_day,'left',on=['user_id','day'])
    user_query_day_hour = data.groupby(['user_id','day','hour']).size().reset_index().rename(column={0:'user_query_day_hour'})
...
if __name__=="__main__":
    ...
    if online == False:
        ...
        train = data.copy()
        test = pd.read_csv('round1_ijcai_18_test_20180301.txt',sep = '')
    ...
    if online ==False:
        ...
        test['predcited_score'] = clf.predict_proba(test[features])[:,1]
        test[['instance_id','predicted_score']].to_csv('baselie.csv',index=False,sep=' ')#保存在线提交结果
